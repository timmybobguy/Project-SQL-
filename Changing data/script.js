var conversionApp = new Vue({
  ...
  methods: {
   ...
   handleEvent(event) {
	...
	var resultSeven = valueSeven.concat(test.slice(25,30))
   ...
   printingResult() {
	var finalOne = ''
	for(let i = 0; i<this.outputData1.length; i++) {
		var last_elementOne = this.outputData1[i].length - 1
	    for(let a = 0; a<this.outputData1[i].length; a++) {
			finalOne += this.outputData1[i][a]
			if (last_elementOne != i) {
				finalOne += ','
			}
		}
